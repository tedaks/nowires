...
export default function ProfileChart({ result }: Props) {
  ...
  const { traces, shapes, annotations } = useMemo(() => {
    if (!profile || profile.length === 0) return { traces: [], shapes: [], annotations: [] };
    ...
    const t: Plotly.Data[] = [fresnelZone, f60Trace, losTrace, terrainTrace];
    ...
    const s: Partial<Plotly.Shape>[] = (horizons || []).map((h) => ({
      type: "line" as const,
      x0: h.d_m / 1000,
      x1: h.d_m / 1000,
      yref: "paper" as const,
    ...
    const a: Partial<Plotly.Annotations>[] = (horizons || []).map(
      (h, i) => ({
        x: h.d_m / 1000,
        y: 1,
        yref: "paper" as const,
