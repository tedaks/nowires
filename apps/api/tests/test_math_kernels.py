...
def test_fresnel_matches_scalar_functions():
    ...
    from app.math_kernels import fresnel_profile_analysis
    from app.p2p import earth_bulge, fresnel_radius
    ...
    k_factor = 4.0 / 3.0
    distances = np.linspace(0, dist_m, n)
    elevations = np.array([10.0 * math.sin(i / n * math.pi) for i in range(n)], dtype=np.float64)
...
def test_apply_coverage_colors_excellent_signal():
    ...
    from app.signal_levels import SIGNAL_LEVELS
